    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <form action="{{ route('voyager.'.$bread->slug.'.delete') }}" id="bulk_delete_form" method="POST">
                    {{ csrf_field() }}
                    <input type="hidden" name="ids" id="bulk_delete_input" value="">
                    <input type="submit" class="btn btn-danger pull-right delete-confirm"
                             value="{{ __('voyager.generic.bulk_delete_confirm') }} {{ strtolower($bread->display_name_plural) }}">
                </form>
...
window.onload = function () {
    ...
    $bulkDeleteBtn.click(function () {
        var ids = [];
        var $checkedBoxes = $('#datatable input[type=checkbox][name=row_id]:checked');
        var count = $checkedBoxes.length;
        if (count) {
            // Reset input value
            $bulkDeleteInput.val('');
            // Deletion info
            var displayName = count > 1 ? '{{ $bread->display_name_plural }}' : '{{ $bread->display_name_singular }}';
